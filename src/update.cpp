...
namespace YAML
{
    template<> struct convert<VersionYAML>
    {
...
constexpr int matchVersionString(const char* text)
{
    ...
    return *text != '\0' && matchDigit(text) && *(text + 1) != '\0' && matchDigit(text + 1) && *(text + 2) != '\0'
        && *(text + 2) == '.' && *(text + 3) != '\0' && matchDigit(text + 3) && *(text + 4) != '\0' && matchDigit(text + 4)
        && *(text + 5) == '\0';
}
...
int Update::getVersionInfo(bool& available, std::string& url)
{
    ...
    static_assert(matchVersionString(VITA_VERSION), "VITA_VERSION=" VITA_VERSION " but must match \\d\\d\\.\\d\\d");
...
void Update::startUpdateThread()
{
    SceUID thid = sceKernelCreateThread(
        "update_check_thread", (SceKernelThreadEntry)Update::updateThread, 0x40, 0x20000, 0, 0, nullptr);
